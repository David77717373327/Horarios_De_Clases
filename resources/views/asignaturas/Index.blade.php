...
<div class="container py-4">
    {{-- Header --}}
    <div class="mb-4">
        <h2 class="fw-bold mb-1" style="color: #000000;">Gestión de Asignaturas</h2>
        <p class="text-muted mb-0" style="font-size: 0.95rem;">Administra las asignaturas del sistema educativo</p>
    </div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h5 class="mb-0 fw-semibold" style="color: #000000; font-size: 1.1rem;">Lista de Asignaturas</h5>
            <small class="text-muted" style="font-size: 0.85rem;">{{ $asignaturas->count() }} {{ $asignaturas->count() === 1 ? 'asignatura registrada' : 'asignaturas registradas' }}</small>
        </div>
        <button onclick="openCreateModal()" class="btn fw-semibold d-inline-flex align-items-center gap-2" 
    ...
    <div class="card border-0 shadow-sm" style="border: 1px solid #e5e7eb;">
        <div class="card-header bg-white border-bottom" style="border-color: #e5e7eb !important; padding: 1.25rem 1.5rem;">
            <h5 class="mb-0 fw-semibold" style="color: #000000; font-size: 1.05rem;">Asignaturas Registradas</h5>
        </div>
                ...
                <table class="table table-hover mb-0" style="font-size: 0.95rem;">
                    <thead style="background-color: #f9fafb; border-bottom: 2px solid #e5e7eb;">
                        <tr>
                            ...
                            <th class="fw-semibold" style="color: #374151; padding: 1rem 1.5rem;">Nombre de la Asignatura</th>
                                ...
                                <td class="align-middle text-center" style="padding: 1rem 1.5rem;">
                                    <div class="btn-group" role="group">
                                        ...
                                        <button 
                                            ...
                                            onclick='confirmDelete({{ $asignatura->id }}, "{{ addslashes($asignatura->nombre) }}", {{ $asignatura->horarios_count ?? 0 }})'
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="text-center" style="padding: 3rem; color: #9ca3af;">
                                    ...
                                    <p class="mb-0 fw-medium" style="font-size: 1rem;">No hay asignaturas registradas</p>
                                    <small class="text-muted">Comienza agregando tu primera asignatura</small>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
...
<div class="modal fade" id="createModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border: none; border-radius: 0.75rem; box-shadow: 0 10px 25px rgba(0,0,0,0.15);">
            ...
            <form method="POST" action="{{ route('asignaturas.store') }}" id="create-form">
                @csrf
                <div class="modal-body" style="padding: 2rem;">
                    <div class="mb-4">
                        <label class="form-label fw-semibold" style="color: #374151; font-size: 0.95rem;">
                            <i class="fas fa-tag me-2" style="color: #6b7280;"></i>Nombre de la Asignatura
                        </label>
                        <input 
                            type="text" 
                            name="nombre" 
                        ...
                        <small class="text-muted d-block mt-2" style="font-size: 0.85rem;">
                            <i class="fas fa-info-circle me-1"></i>Ingresa un nombre descriptivo para la asignatura
                        </small>
                    </div>
                </div>
                ...
                <div class="modal-body" style="padding: 2rem;">
                    <div class="mb-4">
                        ...
                        <input 
                            ...
                            required>
                        <small class="text-muted d-block mt-2" style="font-size: 0.85rem;">
                            <i class="fas fa-info-circle me-1"></i>Modifica el nombre de la asignatura
                        </small>
                    </div>
                </div>
                <div class="modal-footer" style="border-top: 1px solid #e5e7eb; padding: 1.25rem 1.5rem; background-color: #f9fafb;">
...
function openEditModal(id, nombre) {
    ...
    document.getElementById('edit-form').action = "{{ url('asignaturas') }}/" + id;
    ...
    setTimeout(() => {
        ...
        document.getElementById('nombre_edit').select();
    }, 300);
}

// Confirmar eliminación con SweetAlert
function confirmDelete(id, nombre, horariosCount) {
    Swal.fire({
        title: '¿Eliminar asignatura?',
        html: `
            <div style="text-align: left; margin-top: 1rem;">
                <p style="font-size: 1.05rem; margin-bottom: 1rem;">
                    Asignatura: <strong style="color: #1e40af;">${nombre}</strong>
                </p>
                ${horariosCount > 0 ? `
                ...
                <div style="background: #fffbeb; padding: 1rem; border-radius: 0.5rem; border-left: 4px solid #f59e0b;">
                    <p style="margin: 0; color: #92400e;">
                        <i class="fas fa-info-circle me-2"></i>Esta asignatura no tiene horarios y se puede eliminar de forma segura.
                    </p>
                </div>
                `}
            </div>
        ...
        if (result.isConfirmed) {
            ...
            form.action = "{{ url('asignaturas') }}/" + id;
            form.submit();
        }
    });
}
